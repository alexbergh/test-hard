...
if [ "${INSTALL_PRECOMMIT:-true}" = "true" ]; then
    ...
    if python3 -m pip install --user pre-commit; then
        if [ -f ".pre-commit-config.yaml" ]; then
            ...
            log "⚠ .pre-commit-config.yaml not found, skipping hooks installation"
        fi
    else
        log "⚠ Failed to install pre-commit (optional)"
    fi
fi
...
if python3 -c "from atomic_operator.atomic_operator import AtomicOperator" 2>/dev/null; then
    log "✓ atomic-operator available"
else
    log "⚠ atomic-operator not available (will be installed on first use)"
fi
