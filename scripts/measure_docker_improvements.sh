...
if ! grep -q "DOCKER_BUILDKIT" ~/.bashrc 2>/dev/null && ! grep -q "DOCKER_BUILDKIT" ~/.zshrc 2>/dev/null; then
    echo -e "${YELLOW}⚠️  BuildKit не установлен в shell profile${NC}"
...
if [ -n "$cache_size" ]; then
    ...
    if (( $(echo "$cache_num > 5000" | bc -l) )); then
        echo -e "${YELLOW}⚠️  Build cache занимает много места: $cache_size${NC}"
...
if [ "$unused" -gt 0 ]; then
    echo -e "${YELLOW}⚠️  Найдено $unused dangling образов${NC}"
