...
cp -r "$PROJECT_ROOT/.env" "$TEMP_BACKUP/config/" 2>/dev/null || log "⚠ No .env file found"
...
if [ -d "$PROJECT_ROOT/prometheus-data" ]; then
    ...
    log "✓ Prometheus data backed up"
else
    log "⚠ No Prometheus data found (docker volume)"
fi
...
if [ -d "$PROJECT_ROOT/grafana-data" ]; then
    ...
    log "✓ Grafana data backed up"
else
    log "⚠ No Grafana data found (docker volume)"
fi
...
if $COMPOSE_BIN ps prometheus >/dev/null 2>&1; then
    ...
    docker run --rm \
        ...
        log "⚠ Failed to export Prometheus volume"
fi
...
if $COMPOSE_BIN ps grafana >/dev/null 2>&1; then
    ...
    docker run --rm \
        ...
        log "⚠ Failed to export Grafana volume"
fi
