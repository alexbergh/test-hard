...
Usage: podman run test-hard <command> [args]
...
Examples:
  # Run all scanners
  podman run --rm -v /run/podman/podman.sock:/run/podman/podman.sock test-hard scan-all

  # Run Lynis on a specific container
  podman run --rm test-hard lynis audit system

  # Run OpenSCAP scan
  podman run --rm test-hard scan-openscap

  # Run Atomic Red Team tests in dry-run mode
  podman run --rm test-hard atomic --dry-run

  # Parse reports
  podman run --rm -v ./reports:/reports test-hard parse-lynis /reports/lynis.json
