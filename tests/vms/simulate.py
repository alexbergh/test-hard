...
def _simulate_environment(environment: str) -> None:
    ...
    baseline_key = "vm_test" if environment == "test" else "vm_prod"
    baseline = PROCESS_ANALYTICS.baseline_snapshot(baseline_key)
    avg_compliance = sum(item["compliance"] for item in summary) / max(len(summary), 1)
    compliance_ratio = avg_compliance / 100.0
    inventory_density = len(inventory_events) / max(len(summary), 1)
    observed = ProcessMetricSnapshot(
        cycle_time_hours=max(1.0, baseline.cycle_time_hours * (1 - 0.25 * compliance_ratio)),
        review_rounds=max(1.0, baseline.review_rounds * (1 - 0.35 * compliance_ratio)),
        review_comments=max(0.0, baseline.review_comments * (1 - 0.4 * compliance_ratio)),
        incident_frequency=max(0.0, baseline.incident_frequency * (1 - 0.3 * compliance_ratio)),
        mttr_hours=max(0.5, baseline.mttr_hours * (1 - 0.05 * inventory_density)),
        p95_latency_ms=max(120.0, baseline.p95_latency_ms * (1 - 0.02 * min(inventory_density, 5))),
        error_budget_consumed=max(0.0, baseline.error_budget_consumed * (1 - 0.32 * compliance_ratio)),
    )
