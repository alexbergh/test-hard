...
def simulate_openscap() -> dict:
    ...
    total = max(summary["definitions_evaluated"], 1)
    pass_rate = summary["passed"] / total
    baseline = PROCESS_ANALYTICS.baseline_snapshot("docker_openscap")
    observed = ProcessMetricSnapshot(
        cycle_time_hours=max(1.0, baseline.cycle_time_hours * (1 - 0.15 * pass_rate)),
        review_rounds=max(1.0, baseline.review_rounds * (1 - 0.5 * pass_rate)),
        review_comments=max(0.0, baseline.review_comments * (1 - 0.55 * pass_rate)),
        incident_frequency=max(0.0, baseline.incident_frequency * (1 - 0.45 * pass_rate)),
        mttr_hours=max(0.5, baseline.mttr_hours * (1 - 0.2 * pass_rate)),
        p95_latency_ms=max(100.0, baseline.p95_latency_ms * (1 - 0.1 * pass_rate)),
        error_budget_consumed=max(0.0, baseline.error_budget_consumed * (1 - 0.5 * pass_rate)),
    )
    ...
    summary["process"] = process_report.as_dict()
...
def simulate_telemetry() -> dict:
    ...
    baseline = PROCESS_ANALYTICS.baseline_snapshot("docker_telemetry")
    improvement_factor = max(0.5, 1 - severity_ratio * 0.3)
    incident_factor = max(0.3, 1 - min(vulnerability_total, 10) * 0.02)
    latency_factor = max(0.4, 1 - min(metrics.inventory_total, 10) * 0.015)
    observed = ProcessMetricSnapshot(
        cycle_time_hours=max(1.0, baseline.cycle_time_hours * improvement_factor),
        review_rounds=max(1.0, baseline.review_rounds * (1 - severity_ratio * 0.4)),
        review_comments=max(0.0, baseline.review_comments * (1 - severity_ratio * 0.5)),
        incident_frequency=max(0.0, baseline.incident_frequency * incident_factor),
        mttr_hours=max(0.5, baseline.mttr_hours * (1 - min(metrics.unique_hosts, 5) * 0.04)),
        p95_latency_ms=max(80.0, baseline.p95_latency_ms * latency_factor),
        error_budget_consumed=max(0.0, baseline.error_budget_consumed * (1 - min(vulnerability_total, 10) * 0.03)),
    )
    process_report = PROCESS_ANALYTICS.evaluate(
        "docker_telemetry",
        observed,
...
def simulate_wazuh() -> dict:
    ...
    baseline = PROCESS_ANALYTICS.baseline_snapshot("docker_wazuh")
    event_volume = len(alerts) + len(fim_entries)
    event_factor = max(0.45, 1 - event_volume * 0.03)
    observed = ProcessMetricSnapshot(
        cycle_time_hours=max(1.0, baseline.cycle_time_hours * event_factor),
        review_rounds=max(1.0, baseline.review_rounds * event_factor),
        review_comments=max(0.0, baseline.review_comments * event_factor),
        incident_frequency=max(0.0, baseline.incident_frequency * (1 - event_volume * 0.025)),
        mttr_hours=max(0.5, baseline.mttr_hours * (1 - event_volume * 0.015)),
        p95_latency_ms=max(100.0, baseline.p95_latency_ms * (1 - event_volume * 0.01)),
        error_budget_consumed=max(0.0, baseline.error_budget_consumed * (1 - event_volume * 0.02)),
    )
    process_report = PROCESS_ANALYTICS.evaluate(
        "docker_wazuh",
        observed,
