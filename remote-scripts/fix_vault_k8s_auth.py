...
if not tok:
    ...
    if len(keys) >= 2:
        # Generate OTP
        out_init, err_init = run(['kubectl', 'exec', '-n', 'vault', 'vault-0', '--',
        ...
        try:
            ...
            if encoded_token:
                out_dec, _ = run([
                    'kubectl', 'exec', '-n', 'vault', 'vault-0', '--',
                    'vault', 'operator', 'generate-root',
                    f'-otp={otp}', '-decode', encoded_token
                ])
                tok = out_dec.strip()
...
out4, err4 = run(['kubectl', 'exec', '-n', 'vault', 'vault-0', '--',
...
if 'kubernetes' in out4.lower():
    ...
    # Enable
    out5, err5 = run(['kubectl', 'exec', '-n', 'vault', 'vault-0', '--',
                      'env', f'VAULT_TOKEN={tok}', 'vault', 'auth', 'enable', 'kubernetes'])
    print("enable kubernetes:", out5 or err5)

    # Configure
    out6, err6 = run(['kubectl', 'exec', '-n', 'vault', 'vault-0', '--',
                      'env', f'VAULT_TOKEN={tok}', 'vault', 'write',
                      'auth/kubernetes/config', f'kubernetes_host={K8S_HOST}'])
    print("configure k8s host:", out6 or err6)

# ── Verify ────────────────────────────────────────────────────────────────────
out7, _ = run(['kubectl', 'exec', '-n', 'vault', 'vault-0', '--',
               'env', f'VAULT_TOKEN={tok}', 'vault', 'auth', 'list'])
print("\nFinal auth list:")
print(out7[:400])
