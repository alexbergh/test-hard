...
if not tok:
    ...
    if 'kubernetes' in out3.lower():
        ...
        out4, _ = run(['kubectl', 'config', 'view', '--raw',
...
out, _ = run(['kubectl', '-n', 'kube-system', 'rollout', 'status',
              'daemonset', 'kube-proxy', '--timeout=60s'], timeout=70)
print(out)

out2, _ = run(['kubectl', '-n', 'kube-system', 'get', 'cm', 'kube-proxy',
...
items = [
    ...
    ("SA kube-system",        ['kubectl', 'get', 'sa', 'default', '-n', 'kube-system', '-o', 'jsonpath={.automountServiceAccountToken}']),
    ("SA kyverno",            ['kubectl', 'get', 'sa', 'default', '-n', 'kyverno', '-o', 'jsonpath={.automountServiceAccountToken}']),
