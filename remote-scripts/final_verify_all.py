...
def vexec(vault_cmd, timeout=15):
    return run(
        ["kubectl", "exec", "-n", "vault", "vault-0", "--", "env", f"VAULT_TOKEN={VAULT_TOKEN}"] + vault_cmd,
        timeout=timeout,
    )
...
out, _ = run(["kubectl", "-n", "kube-system", "get", "cm", "kube-proxy", "-o", "jsonpath={.data.config\\.conf}"])
...
for ns in ["kube-system", "kyverno", "vault"]:
    out3, _ = run(["kubectl", "get", "sa", "default", "-n", ns, "-o", "jsonpath={.automountServiceAccountToken}"])
