...
checks = [
    ...
    ("vault not sealed", ["kubectl", "exec", "-n", "vault", "vault-0", "--", "vault", "status"]),
    ("kyverno verifyImages policy", ["kubectl", "get", "clusterpolicy", "verify-image-signatures", "--no-headers"]),
...
for label, cmd in checks:
    ...
    if "kube-proxy" in label:
        ...
        ok = out.strip() == "false"
    elif "PSS: kyverno" in label or "PSS: vault" in label:
