...
INDEXER_CERTS="${CONFIG_DIR}/wazuh-indexer/certs"
...
openssl genrsa -out "${INDEXER_CERTS}/wazuh-indexer-key.pem" 4096
openssl req -new -key "${INDEXER_CERTS}/wazuh-indexer-key.pem" \
...
openssl x509 -req -in "${INDEXER_CSR}" -CA "${INDEXER_CERTS}/root-ca.pem" -CAkey "${INDEXER_CERTS}/root-ca.key" -CAcreateserial \
  -out "${INDEXER_CERTS}/wazuh-indexer.pem" -days 1825 -sha256 -extfile "${SAN_FILE}" -extensions v3_req
...
chmod 600 "${INDEXER_CERTS}/root-ca.key" "${INDEXER_CERTS}/wazuh-indexer-key.pem" \
  "${DASHBOARD_CERTS}/dashboard.key" "${MANAGER_SSL}/server.key"
chmod 644 "${INDEXER_CERTS}/root-ca.pem" "${INDEXER_CERTS}/wazuh-indexer.pem" \
