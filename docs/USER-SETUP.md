...
# Для Podman не требуется добавление в группу - rootless по умолчанию
...
LOG_LEVEL=INFO
CONTAINER_HOST=unix:///run/podman/podman.sock
# Используем Podman Socket Proxy для безопасности
EOF
...
# Или через podman exec
podman exec grafana grafana-cli admin create-user \
  --login hardening-readonly \
  --email readonly@test-hard.local \
  --password CHANGE_THIS_PASSWORD \
  --role Viewer
...
| Пользователь | Rootless | Socket Proxy | Разрешенные команды |
|--------------|--------------|--------------|---------------------|
| hardening-admin | Да | Нет | Все |
| hardening-scanner | Да | Да | podman exec (ограниченно) |
| hardening-service | Да | Да | podman ps, podman exec |
| hardening-readonly | Да | Нет | Нет |

### Sudo права

| Пользователь | Права sudo | Команды |
|--------------|------------|---------|
| hardening-admin | Ограниченные | podman, podman-compose, systemctl restart podman |
| hardening-scanner | Ограниченные | lynis, oscap, podman exec */lynis*, podman exec */oscap* |
...
- [ ] Используется rootless Podman (или Socket Proxy)
...
- [Podman Security Best Practices](https://docs.podman.io/en/latest/markdown/podman.1.html)
...
**Q: Нужно ли запускать Podman от root?**  
A: **НЕТ!** Podman работает в rootless режиме по умолчанию. Используйте Podman Socket Proxy для дополнительной безопасности.
