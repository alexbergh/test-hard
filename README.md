...
автоматически без дополнительных запросов к API. Графана подставляет адрес из переменной окружения `PROMETHEUS_URL`
(по умолчанию `http://prometheus:9090`), поэтому datasource не будет «пустым», даже если переменная не задана.
